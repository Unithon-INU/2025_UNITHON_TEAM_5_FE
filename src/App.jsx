...
import HospitalDetailContent from "./components/HospitalDetailContent";
function App() {
  // i18n 초기화
  ...
  useEffect(() => {
    ...
    if (DeptDropdown) {
      ...
      // 드롭다운이 열리면 mousedown 이벤트 리스너를 document에 추가
      document.addEventListener("mousedown", handleClickOutside);
      // cleanup함수 : useEffect가 다시 실행되거나 컴포넌트가 unmount될 때 리스너를 제거한다.
      // 드롭다운이 닫히면(DeptDropdown이 false가 되면) return 문이 실행되어 추가했던 이벤트 리스너를 제거
      return () => {
